...
#include <stdlib.h>
#include <time.h>
#include <math.h> // Para la función exponencial y tanh
...
double activacion(double *pesos, int *entrada, double b) {
    double suma = 0.0;
    for (int i = 0; i < NUM_ENTRADAS; i++) {
        suma += pesos[i] * entrada[i]; // Multiplicamos los pesos por las entradas
    }
    suma += b; // Añadimos el sesgo
    return tanh(suma); // Aplicamos la función tanh
}
...
int main() {
    ...
    printf("Resultados del perceptrón para la tabla AND (usando activación tangente hiperbólica):\n");
    ...
    for (int epoca = 0; epoca < EPOCAS; epoca++) {
        ...
        for (int i = 0; i < 4; i++) {
            // Predicción
            double prediccion = activacion(pesos, entradas[i], b);
